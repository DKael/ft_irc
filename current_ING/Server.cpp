    ...
    if (event_cnt == 0) {
      ...
      for (int i = 1; i < MAX_USER && event_cnt > 0; i++) {
        if (observe_fd[i].fd > 0) {
          ...
          if (observe_fd[i].revents & (POLLIN | POLLHUP)) { // 소켓연결이 끊어졌을 경우 발생
...
void Server::cmd_nick(int recv_fd, const Message& msg) {
  ...
  else 
  {
    ...
    //   std::cout << YELLOW << "===================>> " << nick_tmp << WHITE << std::endl;
    // }
    // else
    // {
    //   nick_tmp = msg[0];
        ...
        || nick_tmp.length() > max_nickname_len)
    {
      ...
      event_user.push_msg(Message::rpl_432(serv_name, event_user.get_nick_name(), nick_tmp).to_raw_msg());
      return;
    }
    try {
      (*this)[nick_tmp];
      event_user.push_msg(Message::rpl_433(serv_name, event_user.get_nick_name(), nick_tmp).to_raw_msg());
      return;
    } catch (std::invalid_argument& e) {
      if (event_user.get_is_authenticated() == OK) {
        Message rpl;

        rpl.set_source(event_user.get_nick_name() + "!" + event_user.get_user_name() + "@localhost");
