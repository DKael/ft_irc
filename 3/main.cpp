      ...
      if (event_cnt == 0) {
        ...
        if (observe_fd[0].revents & POLLIN)
        {
          while (true)
          {
            ...
            // std::cout << "user_addr_len :: " << user_addr_len << std::endl;
            // std::cout << "user_addr :: " << &user_addr << std::endl;
            // std::cout << "user_socket :: " << user_socket << std::endl;
            ...
            for (int i = 1; i < MAX_USER; i++) {
              if (observe_fd[i].fd == -1) {
                ...
                std::cout << "\t"<< observe_fd[i].fd << std::endl;
                observe_fd[i].events = POLLIN;
                std::cout << "[" << i << "]" << "client socket connected!" << std::endl;
                break;
              }
            }
          }
