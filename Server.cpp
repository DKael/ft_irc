...
Server::Server(const char* _port, const char* _password)
  ...
  int Server::client_socket_init(void) {
    ...
    while (connection_limit != 0) {
      ...
      user_socket =
          ::accept(serv_socket, (sockaddr*)&user_addr, &user_addr_len);
      if (user_socket == -1) {
  ...
  void Server::cmd_kick(int recv_fd, const Message& msg) {
    ...
    try {
      ...
      User& event_user = (*this)[recv_fd];
      Message rpl =
          Message::rpl_401(serv_name, event_user.get_nick_name(), msg);
      event_user.push_msg(rpl.to_raw_msg());
    }
  }
  ...
  void Server::not_auth_user(pollfd & p_val,
                             std::vector<std::string> & msg_list) {
    User& event_user = (*this)[p_val.fd];
  ...
  const int Server::get_tmp_user_cnt(void) const {
    return tmp_user_list.size();
  }
  ...
  void Server::add_tmp_user(const int user_socket,
  ...
  void Server::cmd_privmsg(int recv_fd, const Message& msg) {
    ...
    try {
      ...
      source_user.push_msg(Message::rpl_401(serv_name,
                                            source_user.get_nick_name(),
                                            targetNickName)
                               .to_raw_msg());
      return;
    }
